...
\usepackage[subsection]{placeins}   % barrier at \subsection too
\usepackage{pgfkeys}
%%%%%%%%%%%%%%%%%%%%5


%\graphicspath{./Images/}
\newcommand{\vect}[1]{\mathbf{#1}}   % or \boldsymbol{#1}
...
\newcommand{\plotAnalysis}[6]{
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
			\caption{Modified \AlgoName}
			\label{fig:\AlgoName_ctrl_mod_xb}
		\end{subfigure}
		...
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
			\caption{\AlgoName}
			\label{fig:\AlgoName_ctrl_base_xb}
		\end{subfigure}

		\caption{Control trajectories for $x_0=\xB$ with
		$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_ctrl_compare_xb}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{../Images/Projection_Comparison_PyPlots/high_Difference_Comparison_PyPlots/Projection_Comparison_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
		\caption{Trajectory comparison for $x_0=\xB$ with
		$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_proj_xb}
	\end{figure}
}

\newcommand{\algoAnalysis}[1][]{
	\pgfkeys{/algoAnalysis, method=, k=1.0, h=10.0, N=100, m=1.0, xA=, xB=, xC=, #1}
	...
	\edef\xB{\pgfkeysvalueof{/algoBnalysis/xB}}
	\edef\xB{\pgfkeysvalueof{/algoCnalysis/xC}}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[width=\textwidth]{../Images/Iteration_PyPlots/Full/IterationPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval}
		\end{subfigure}
		...
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[width=\textwidth]{../Images/Iteration_PyPlots/Full/IterationPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval}
		\end{subfigure}

		\caption{Iteration plots for \AlgoName and its modified variant with
		$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_iter_compare}
	\end{figure}
...
\begin{align}
    ...
    \textbf{Modified MidPoint:} $(\mathbf{M.M.P})$
    \begin{align}
	...
	\label{subsec:numerical-results-(nonlinear-case)}
...
\multirow{3}{*}{$SE2_1$} & $0.02$ & $1.1769\times 10^{-5}$ & $9.415\times 10^{-3}$ & $1.2610\times 10^{-4}$ &$0.1008$ \\
                     ...
                     \hline
%\cline{2-5}
\multirow{3}{*}{MidPoint}  &$0.02$&$1.7307\times 10^{-5}$& $1.3845\times 10^{-2}$& $2.0385\times 10^{-4}$ &  $0.1630$ \\
...
\pagebreak
	...
	\section{Algorithm Comparison}
		In this section, we will compare the different algorithms (SE1, SE2, MidPoint) to their Modified Counterparts.
		We will do this comparison by introduction three types of graphs:
		\begin{itemize}
			\item \textbf{Iteration Plots:} are plots that show us for some combination of parameters <$k$, $m$, $N$, $\alpha$, $x_d$> how many iterations
			it took to converge, if we did converge, for some values $x_0$ represented by dots. And on top of that it will show us the arrow of from each $x_0$ to
			the final reached position $x_N$
			\item \textbf{Control Plots:} are plots that show the control function $u(t)$ over the iteration.
			\item \textbf{Trajectory Plots:} are plots that show the path that the pendulum bob took while going from $x_0$ to $x_N$
		\end{itemize}


		\noindent Before we go and look at the results lets first take a look at how an iteration plot looks like and explain it in further detail:

		\begin{figure}[h!]
			\centering
			\includegraphics[width=\textwidth]{../Images/Iteration_PyPlots/Full/IterationPlot_Modified SE1_k=0.25_h=10.0_N=200_m=1.0}
			\caption{Iteration plot of Modified SE1 for k=0.25 h=10.0 N=200 and m=1.0}
			\label{fig:example_iter_plot}
		\end{figure}

		\noindent Figure~\ref{fig:example_iter_plot} shows an iteration plot of the \emph{Modified SE1} algorithm with parameter values of $k=0.25$, $h=10.0$, $N=200$, $m=1.0$, $x_d = (0.00, 10.00)$. The
		iteration plot contains multiple dots in the input space, these are different combination of the initial state $x_0$. The color of these dots shows how long it took for the algorithm
		to converge, if the algorithm didn't converge in the predefined number of maximum iteration, 30 for our case here, then the dots are colored black. On top of the color
		there are also arrows coming out of the dots, and they point to the final position $x_N$ of the system. Keep in mind they don't show the trajectory through the state space
		but the projection plot does.

	 	\pagebreak
		\begin{figure}[h!]
			\centering
			\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_Modified SE1_k=0.25_h=10.0_N=200_m=1.0_x0=-1.11, 2.12}
			\caption{Control plot of Modified SE1 for k=0.25 h=10.0 N=200, m=1.0, and $\mathbf{x_0}= (-1.11, 2.12)$}
			\label{fig:example_control_plot}
		\end{figure}

		\noindent Figure~\ref{fig:example_control_plot} shows the control function of the Modified SE1 for the parameter values of $k=0.25$, $h=10.0$, $N=200$, $m=1.0$, $x_d = (0.00, 10.00)$
		and initial value $x_0 = (-1.11, 2.12)$. For modified algorithms the x axis represents some time $t_i$ where we do not know the exact time $t_i$ but as $N \rightarrow \infty$, $t_i \rightarrow t_0 + \frac{T - t_0}{N} * i$.
		And for unmodified algorithms $t_i = t_0 + \frac{T - t_0}{N} * i$. Where $i \in [0, N]$


		\begin{figure}[h!]
			\centering
			\includegraphics[width=.8\textwidth]{../Images/Projection_PyPlots/ProjectionPlot_Modified SE1_k=0.25_h=10.0_N=200_m=1.0_x0=-1.11, 2.12}
			\caption{Trajectory plot of Modified SE1 for k=0.25 h=10.0 N=200, m=1.0, and $\mathbf{x_0} = (-1.11, 2.12)$}
			\label{fig:example_projection_plot}
		\end{figure}

		\noindent Figure~\ref{fig:example_projection_plot} shows the trajectory plot of the Modified SE1 for the parameter values of $k=0.25$, $h=10.0$, $N=200$, $m=1.0$, $x_d = (0.00, 10.00)$
		and initial value $x_0 = (-1.11, 2.12)$. The trajectory plots shows the trajectory through the state space that the pendulum bob took to get to its final position as marked on the Iteration Plot (Figure~\ref{fig:example_iter_plot}) \newline

		\noindent One important detail before we go further, since there are so many examples we only showcased the ones where the difference in the projections were high. Another thing to point out quickly is that when the height
		of the desired point was small, the converged results tend to make the pendulum bob fall, and for some reason this also made a drastic difference in the projection even if the end positions were pretty similar.
...
{
	% --------------------------
	% Parameters for this subsection
	% --------------------------
	\newcommand{\AlgoName}{SE1} % e.g. MidPoint
	\newcommand{\kval}{1.0}
	\newcommand{\hval}{8.0}
	\newcommand{\Nval}{200}
	\newcommand{\mval}{1.0}

	\newcommand{\xA}{2.00, 2.93}
	\newcommand{\xB}{-1.56, 2.93}

	% Optional: desired target if you reference it in text
	% \newcommand{\xd}{(0.00,\;10.00)}

	% --------------------------
	% Iteration plots (Modified vs Base)
	% --------------------------
	\begin{figure}[H]
	  \centering
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.98\linewidth]{../Images/Iteration_PyPlots/Full/IterationPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval}
		\label{fig:\AlgoName_iter_mod}
	  \end{subfigure}
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.98\linewidth]{../Images/Iteration_PyPlots/Full/IterationPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval}
		\label{fig:\AlgoName_iter_base}
	  \end{subfigure}
	...
	% --------------------------
	% Control plots for x0 = xA (Modified vs Base)
	% --------------------------
	\begin{figure}[H]
	  \centering
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.98\linewidth]{../Images/Control_PyPlots/ControlPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xA.pdf}
		\caption{Modified~\AlgoName}
		\label{fig:\AlgoName_ctrl_mod_xa}
	  \end{subfigure}
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.98\linewidth]{../Images/Control_PyPlots/ControlPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xA.pdf}
		\caption{\AlgoName}
		\label{fig:\AlgoName_ctrl_base_xa}
	  \end{subfigure}

	  \caption{Control trajectories for $x_0=\xA$ with
	  $k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
	  \label{fig:\AlgoName_ctrl_compare_xa}
	\end{figure}

	% --------------------------
	% Trajectory comparison for x0 = xA
	% --------------------------
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{../Images/Projection_Comparison_PyPlots/high_Difference_Comparison_PyPlots/Projection_Comparison_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xA}
		\caption{Trajectory comparison for $x_0=\xA$ with
			$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_proj_xa}
	\end{figure}
	\vfill

	% --------------------------
	% Control plots for x0 = xB (Modified vs Base)
	% --------------------------
	\begin{figure}[H]
	  \centering
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.98\linewidth]{../Images/Control_PyPlots/ControlPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
		\caption{Modified~\AlgoName}
		\label{fig:\AlgoName_ctrl_mod_xb}
	  \end{subfigure}
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.98\linewidth]{../Images/Control_PyPlots/ControlPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
		\caption{\AlgoName}
		\label{fig:\AlgoName_ctrl_base_xb}
	  \end{subfigure}
	...
	% --------------------------
	% Trajectory comparison for x0 = xB
	% --------------------------
	\begin{figure}[H]
		\centering
		\includegraphics[height=.9\textheight]{../Images/Projection_Comparison_PyPlots/high_Difference_Comparison_PyPlots/Projection_Comparison_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
		\caption{Trajectory comparison for $x_0=\xB$ with
			$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_proj_xb}
	\end{figure}
...
% --------------------------
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{../Images/Projection_Comparison_PyPlots/Low_Difference_Comparison_PyPlots/Projection_Comparison_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xA}
		\caption{Trajectory comparison for $x_0=\xA$ with
			$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_proj_xa}
	\end{figure}
...
% Parameters for this subsection
% --------------------------
	\newcommand{\AlgoName}{SE2} % e.g. MidPoint
	\newcommand{\kval}{1.0}
	\newcommand{\hval}{6.0}
	\newcommand{\Nval}{100}
	\newcommand{\mval}{1.0}

	\newcommand{\xA}{1.11, 2.52}
...
% --------------------------
	...
	\newcommand{\xA}{-2.00, 3.74}
	\newcommand{\xB}{2.00, 3.74}
...
% --------------------------
	\begin{figure}[H]
		\centering
		\includegraphics[height=.9\textheight]{../Images/Projection_Comparison_PyPlots/Low_Difference_Comparison_PyPlots/Projection_Comparison_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xB}
		\caption{Trajectory comparison for $x_0=\xB$ with
			$k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
		\label{fig:\AlgoName_proj_xb}
	\end{figure}
...
\subsection{MidPoint}
{
	...
	% Parameters for this subsection
	% --------------------------
	\newcommand{\AlgoName}{MidPoint} % e.g. MidPoint
	\newcommand{\kval}{1.0}
	\newcommand{\hval}{10.0}
	\newcommand{\Nval}{200}
	\newcommand{\mval}{1.0}

	\newcommand{\xA}{1.11, 3.74}
	\newcommand{\xB}{2.00, 3.74}
	\newcommand{\xC}{-2.00, 3.74}
	...
	\begin{figure}[H]
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xA.pdf}
		\caption{Modified \AlgoName}
		\label{fig:\AlgoName_ctrl_mod_xa}
	  \end{subfigure}
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xA.pdf}
		\caption{\AlgoName}
		\label{fig:\AlgoName_ctrl_base_xa}
	  \end{subfigure}
...
% --------------------------
	...
	% --------------------------
	% Control plots for x0 = xC (Modified vs Base)
	% --------------------------
	\begin{figure}[H]
	  \centering
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_Modified \AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xC}
		\caption{Modified \AlgoName}
		\label{fig:\AlgoName_ctrl_mod_xc}
	  \end{subfigure}
	  ...
	  \begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../Images/Control_PyPlots/ControlPlot_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xC}
		\caption{\AlgoName}
		\label{fig:\AlgoName_ctrl_base_xc}
	  \end{subfigure}

	  \caption{Control trajectories for $x_0=\xC$ with
	  $k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
	  \label{fig:\AlgoName_ctrl_compare_xc}
	\end{figure}
	...
	% --------------------------
	% Trajectory comparison for x0 = xC
	% --------------------------
	\begin{figure}[H]
	  \centering
	  \includegraphics[width=\textwidth]{../Images/Projection_Comparison_PyPlots/high_Difference_Comparison_PyPlots/Projection_Comparison_\AlgoName_k=\kval_h=\hval_N=\Nval_m=\mval_x0=\xC}
	  \caption{Trajectory comparison for $x_0=\xC$ with
	  $k=\kval$, $h=\hval$, $N=\Nval$, $m=\mval$.}
	  \label{fig:\AlgoName_proj_xc}
	\end{figure}
}
