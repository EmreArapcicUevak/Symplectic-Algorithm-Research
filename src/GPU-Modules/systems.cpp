...
inline void SE1_launch(const double* Y_d, double* R_d, const SE1Params& p,
                       hipStream_t stream = 0) {
  constexpr int BLOCK = 256;

  dim3 grid((p.N + BLOCK - 1) / BLOCK);
  hipLaunchKernelGGL(SE1_loop_kernel, grid, dim3(BLOCK), 0, stream, Y_d, R_d,
                     p.N, p.alpha, p.m, p.k, p.a[0], p.a[1], p.t0, p.T, p.x0[0],
                     p.x0[1], p.l0, p.xd[0], p.xd[1]);

  hipLaunchKernelGGL(SE1_boundary_kernel, dim3(1), dim3(1), 0, stream, Y_d, R_d,
                     p.N, p.alpha, p.m, p.k, p.x0[0], p.x0[1], p.l0);
